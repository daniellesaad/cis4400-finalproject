...
location as (
    select location_dim_id, incident_zip
    from {{ ref('location_dim' )}}
),

fact_data as (
   SELECT agency_name, created_date, complaint_type, incident_zip
   FROM `cis-4400-group-project.filtered_311_table.final_complaint_data`
),

final as (
  select agency_dim_id, date_dim_id, description_dim_id, location_dim_id
  from fact_data 
